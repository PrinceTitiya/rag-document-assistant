...
This repository implements a Retrieval-Augmented Generation (RAG) pipeline on top of PDF documents (for example, blockchain/Ethereum whitepapers). It uses sentence-transformer embeddings, a Chroma vector store, and Groq (Llama 3.3 70B) as the LLM.
...
- **Groq-based answering**: uses `llama-3.3-70b-versatile` via LangChain.

## Project Structure

- **`src/config.py`**: paths, embedding model, LLM model, and `GROQ_API_KEY` loading.
- **`src/pipeline.py`**: `RAGPipeline` orchestration (loader → embedder → vector store → retriever → generator).
- **`src/generator.py`**: `RAGGenerator` that builds prompts and calls Groq.
...
- **`EMBEDDING_MODEL_NAME`**: sentence-transformer model (default `BAAI/bge-small-en-v1.5`).
- **`LLM_MODEL_NAME`**: Groq model used for generation (default `llama-3.3-70b-versatile`).
